 ...
 * darkpay-core: read the cookie file in a loop (singleton!)
 */
export class CoreCookieService {
    ...
    private getCookieLoop(): void {
        try {
            ...
            // we might not be running the darkpayd locally so the cookie might not exists
            if (cookie) {
    ...
    private checkIfExists(dir: string): boolean {
        try {
            ...
            this.log.info('Found darkpay-core path', dir);
            return true;
        } catch (err) {
            this.log.error('Could not find darkpay-core path!', dir);
        }
        return false;
    }
}
